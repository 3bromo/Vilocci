...
// ---------------------------------------------------------------------------
// PRODUCT COLORS — the one color data model for the whole site
// ----------------------------------------------------------------------------
// Every product carries its OWN ordered list of color variants:
//   { id, name_en, name_ar, hex, enabled }
// Array position IS the display order (admins reorder colors by moving entries).
// `enabled: false` keeps a color in the admin list without showing it on the
// storefront. The same normalized list serves the storefront, the admin panel,
// cart/checkout validation and the Customize flow — nothing is hard-coded in
// the frontend, and there is no second color system anywhere.
// ---------------------------------------------------------------------------
const HEX_RE = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
const MAX_COLORS_PER_PRODUCT = 50;

function colorId(input, index) {
  const s = String(input || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '');
  return s || ('color_' + (index + 1));
}

// Accepts a raw JS string, a parsed array, or null — returns a clean, ordered
// array of { shape, available } objects.
function normalizeKeyShapes(raw) {
  let list = raw;
  ...
  list.forEach((s) => {
    if (!s) return;
    const name = (typeof s === 'string' ? s : String(s.shape || '')).trim().toUpperCase();
    if (!name || seen[name]) return;
    seen[name] = true;
    const available = typeof s === 'object' ? (s.available !== false && s.inStock !== false) : true;
    out.push({ shape: name, available });
  });
  return out;
}

// The available slice of a product's key shapes, in admin order.
function availableKeyShapes(product) {
  const raw = product && (product.keyShapes !== undefined ? product.keyShapes : product.key_shapes);
  return normalizeKeyShapes(raw).filter((s) => s.available);
}

// Accepts a raw JS string (json drivers), a parsed array, or null — returns a
// clean, ordered array of color objects. Bad entries are dropped, never fatal.
function normalizeColors(raw) {
  ...
  list.forEach((c, i) => {
    ...
    if (!HEX_RE.test(hex)) return;                 // a color needs a real HEX
    const nameEn = String(c.name_en || '').trim().slice(0, 60);
    const nameAr = String(c.name_ar || '').trim().slice(0, 60);
    if (!nameEn && !nameAr) return;                 // …and a name
    let id = colorId(c.id, i);
    while (seen[id]) id += '_x';
    seen[id] = true;
    out.push({
      id,
      ...
      name_ar: nameAr,
      hex: hex.toUpperCase(),
      enabled: c.enabled !== false && c.active !== false, // default ON
    });
    if (out.length >= MAX_COLORS_PER_PRODUCT) return out;
  });
  return out;
}

// The enabled slice of a product's colors, in admin order. This is exactly
// what the storefront renders and what checkout validates against.
function activeColors(product) {
  return normalizeColors(product && product.colors).filter((c) => c.enabled);
}

// Resolves a customer/admin color reference (an id or a full color object)
// against a product's enabled colors. Returns the stored color or null.
function resolveProductColor(product, ref) {
  const list = activeColors(product);
...
const COLUMNS = {
  ...
  products: [
    ...
    'models', 'years', 'specs', 'vehicles', 'colors',
  ],
  categories: [
...
function productToRow(p) {
  ...
  const row = {
    ...
    sku: str(p.sku),
    key_shapes: json(normalizeKeyShapes(p.keyShapes !== undefined ? p.keyShapes : p.key_shapes), []),
    images,
    ...
    // Per-product color variants (Admin → Products → Colors). Normalized here
    // so whatever the admin panel sends is exactly what gets stored in every
    // driver (jsonb on Supabase, app shape in the JSON fallback).
...
function rowToProduct(r) {
  ...
  return Object.assign({}, r, {
    ...
    keyShapes: normalizeKeyShapes(r.key_shapes),
...
// ---------------------------------------------------------------------------
// KEY SHAPES — Shape management: the global shape catalogue
// ---------------------------------------------------------------------------
// Every key in the store fits one of a small set of physical key shapes
// (A–D today). Before this catalogue existed, the letters lived only as
// per-product availability flags (products.key_shapes) and as hard-coded
// options in the storefront and the Fitment Finder. The catalogue is the
// master list Admin → Shapes manages: the letters, their localized display
// names, their display order and whether customers may pick them at all.
//
// Products keep their own per-shape availability flags; a shape can only be
// bought when the catalogue says it exists AND is active AND the product
// carries it as available. The storefront and the server enforce the same
// rule, so hiding a shape in Admin removes it from every selector at once.
// ---------------------------------------------------------------------------
function shapeCode(input) {
  return String(input || '').trim().toUpperCase();
}

function shapeId(input, code) {
  const id = String(input || '').trim();
  if (id) return id;
  return code ? `shape_${code.toLowerCase()}` : null;
}

function shapeImageUrl(s) {
  const v = s && (s.image_url !== undefined ? s.image_url : s.imageUrl);
  if (v === undefined || v === null) return '';
  return String(v).trim();
}

// App-shape record -> SQL row. Bad entries collapse to a row with a null id,
// which saveRecord rejects with a clear message — a shape without a code can
// never reach the database.
function shapeToRow(s) {
  const code = shapeCode(s && s.code);
  return pick({
    id: shapeId(s && s.id, code),
    code: code || null,
    name_en: str(s && s.name_en),
    name_ar: str(s && s.name_ar),
    description_en: str(s && s.description_en),
    description_ar: str(s && s.description_ar),
    active: bool(s && s.active, true),
    order: num(s && s.order) || 0,
    image_url: shapeImageUrl(s) || null,
  }, COLUMNS.key_shapes);
}
...
function rowToShape(r) {
  ...
  return Object.assign({}, r, {
    ...
    name_ar: str(r.name_ar),
    active: r.active !== false,
    order: Number(r.order || 0),
    image_url: shapeImageUrl(r),
  });
}

// Cleans a raw list of shape records the same way normalizeColors cleans
// colors: drops entries without a code, upper-cases codes, de-duplicates
// them (first wins) and keeps the admin's order. Used by the JSON driver on
// save, so both drivers store the exact same normalized list.
function normalizeShapes(raw) {
  ...
  list.forEach((s, i) => {
    ...
    const code = shapeCode(s.code);
    if (!code) return;                    // a shape needs a code
    if (seen[code]) return;               // one row per code
    seen[code] = true;
    out.push({
      id: shapeId(s.id, code),
      code,
      name_en: String(s.name_en || '').trim(),
      name_ar: String(s.name_ar || '').trim(),
      description_en: String(s.description_en || '').trim(),
      description_ar: String(s.description_ar || '').trim(),
      active: s.active !== false,
      order: Number(s.order != null ? s.order : i + 1),
      image_url: shapeImageUrl(s),
    });
  });
  return out;
}

// The catalogue the store ALWAYS has, even before Admin ever opens the
// Shapes screen: derived from the shapes the live products already carry
// (plus every fitment shape brand models reference), in first-seen order.
// Same pattern categories use — the runtime (lib/db.js), the migration
// (scripts/migrate.js) and the JSON store all agree on the same codes, and
// the default names are exactly the labels the storefront has always shown
// ('Shape A' / 'الشكل A'), so deriving never changes what customers see.
function deriveShapes(db) {
  const codes = [];
  const push = (c) => {
    const code = shapeCode(c);
    if (code && !codes.includes(code)) codes.push(code);
  };
  ((db && db.products) || []).forEach((p) => {
    (p.keyShapes || []).forEach((s) => push(s && s.shape));
  });
  ((db && db.brands) || []).forEach((b) => {
    (b.models || []).forEach((m) => (m.shapes || []).forEach(push));
  });
  return codes.map((code, i) => shapeToRow({
    id: `shape_${code.toLowerCase()}`,
    code,
    name_en: `Shape ${code}`,
    name_ar: `الشكل ${code}`,
    description_en: '',
...
// The active codes of a catalogue, as a Set — used where only catalogue
// codes matter (e.g. the Fitment Finder). An empty/missing catalogue means
// "no restriction" (legacy datasets keep behaving exactly as before).
function activeShapeCodes(shapes) {
  const list = Array.isArray(shapes) ? shapes : [];
  return new Set(list.filter((s) => s && s.active !== false).map((s) => shapeCode(s.code)));
}

// Order-time shape gate. The Products editor can attach CUSTOM shapes that
// are not part of the catalogue (Admin → Products → "+ Add shape"), so the
// rule is: a code NOT listed in the catalogue is a product-local shape and
// is allowed as-is; a code that IS listed must not be hidden there. An empty
// catalogue (legacy data, migration 007 pending) allows everything.
function shapeAllowed(shapes, code) {
  const list = Array.isArray(shapes) ? shapes : [];
  const c = shapeCode(code);
  if (!c) return true;                    // no shape on this line
  if (!list.length) return true;          // no catalogue → no restriction
  const def = list.find((s) => shapeCode(s && s.code) === c);
...
function orderToRow(o) {
  ...
  const explicitPaymentStatus = str(o.paymentStatus !== undefined ? o.paymentStatus : o.payment_status);
  const computedPaymentStatus = explicitPaymentStatus || (row.payment === 'InstaPay' ? 'Pending Verification' : null);
  // Keep COD writes compatible with databases that have not applied the
  // additive payment-status migration yet. InstaPay writes include the status
  // because admin verification depends on it.
  if (computedPaymentStatus && !(row.payment !== 'InstaPay' && computedPaymentStatus === 'Not Required')) {
    row.payment_status = computedPaymentStatus;
  }
  // Keep the new proof columns out of legacy/COD writes when they are not
  // needed. That preserves COD compatibility even while an operator is
  // applying the additive production migration; InstaPay writes include all
  // four columns and therefore fail safely until the migration is present.
  if (o.paymentProof && (o.paymentProof.path || o.paymentProof.data || o.paymentProof.mime)) {
    Object.assign(row, {
      payment_proof_path: str(o.paymentProof.path || o.paymentProof.paymentProofPath),
      payment_proof_mime: str(o.paymentProof.mime),
      payment_proof_size: num(o.paymentProof.size),
      payment_proof_data: str(o.paymentProof.data || o.paymentProof.inlineData),
    });
  }
  // Nano Ceramic Coating fee (migration 006) — same pattern: the column is
  // only written when a coating was actually charged, so orders without the
  // extra never touch it and keep working even before the migration is
  // applied; coated orders fail safely until it is.
  const coatingFee = num(o.coatingFee !== undefined ? o.coatingFee : o.coating_fee) || 0;
  if (coatingFee > 0) row.coating_fee = coatingFee;
  return pick(row, COLUMNS.orders);
}

function rowToOrder(r, items, opts = {}) {
  if (!r) return null;
  ...
  const out = Object.assign({}, r, {
    ...
    // absent column (pre-migration read) or absent fee simply means no
    // coating was charged — never NaN, never undefined leaking to the UIs
    coatingFee: Number(r.coating_fee || r.coatingFee || 0),
    deliveryFee: Number(r.delivery_fee || 0),
    total: Number(r.total || 0),
    statusHistory: Array.isArray(r.status_history) ? r.status_history : [],
    paymentStatus: r.payment_status || (r.payment === 'InstaPay' ? 'Pending Verification' : 'Not Required'),
  });
  const proofPath = r.payment_proof_path || null;
  const proofData = r.payment_proof_data || null;
  // Do not copy private storage fields from the SQL row into the API object.
  // The explicit paymentProof summary below is the only safe representation.
  delete out.payment_proof_path;
  delete out.payment_proof_mime;
  delete out.payment_proof_size;
  delete out.payment_proof_data;
  if (proofPath || proofData) {
    out.paymentProof = {
      available: true,
      path: proofPath,
      mime: r.payment_proof_mime || 'image/jpeg',
      size: Number(r.payment_proof_size || 0),
      storage: proofPath ? 'supabase' : 'inline',
    };
    // Inline proof data is returned only for the authenticated order-detail
    // endpoint, never in the Orders list or public payload.
    if (opts.includePaymentProofData && proofData) out.paymentProof.data = proofData;
  } else {
    out.paymentProof = null;
  }
...
function orderItemToRow(item, orderId, index) {
  return pick({
    ...
    // The exact color variant the customer picked (snapshot — an order keeps
    // showing the chosen color even if the admin later renames/removes it).
    color: item.color && typeof item.color === 'object' && item.color.hex ? json({
      id: str(item.color.id) || '',
      name_en: str(item.color.name_en) || '',
      name_ar: str(item.color.name_ar) || '',
      hex: str(item.color.hex) || '',
    }, null) : null,
    // Nano Ceramic Coating opt-in (migration 006) — snapshot per line, only
    // written when true (same migration-safety pattern as orders.coating_fee:
    // un-coated lines never touch the column, coated ones fail safely until
    // the migration is present). `pick` drops the undefined otherwise.
    coating: item.coating === true ? true : undefined,
  }, COLUMNS.order_items);
}
...
function rowToOrderItem(r) {
  ...
  return {
    ...
    image: r.image || '',
    fitment: r.fitment || {},
    color: parseItemColor(r.color),
    // absent column (pre-migration) or absent flag = no coating requested
    coating: !!r.coating,
  };
}

// A legacy item (pre-colors) or an item for a product without colors has no
// color — reported as null, never as an empty object.
function parseItemColor(raw) {
  let c = raw;
  if (typeof c === 'string') {
    try { c = JSON.parse(c); } catch (e) { c = null; }
  }
  if (!c || typeof c !== 'object' || !c.hex) return null;
  return { id: c.id || '', name_en: c.name_en || '', name_ar: c.name_ar || '', hex: c.hex };
}
...
module.exports = {
  COLUMNS,
  normalizeKeyShapes, availableKeyShapes,
  normalizeColors, activeColors, resolveProductColor,
  shapeToRow, rowToShape, normalizeShapes, deriveShapes, activeShapeCodes, shapeAllowed, shapeCode,

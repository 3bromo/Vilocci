...
// ---------------------------------------------------------------------------
// SHAPE IMAGES — schema probe + best-effort self-migration
// ---------------------------------------------------------------------------
// The feature needs exactly one additive DDL statement (migration 009):
//
//   alter table public.key_shapes add column if not exists image_url text;
//
// The rest of the feature (the public `shape-images` bucket + its Storage
// policies) lives in the same 009 file and is applied by the operator in
// Supabase, exactly like every other migration in this project. `shapeImageColumnPresent()` answers the
// "is 009 applied?" question without guessing, and `ensureShapeImageColumn()`
// applies the statement itself *only when a credential that can run DDL is
// available* (a Postgres connection string, the Supabase Management API token,
// or an operator-installed `exec_sql` RPC). It never throws: the caller gets
// { ok, mode, message } and decides what to tell the admin.
// ---------------------------------------------------------------------------
const SHAPE_IMAGE_DDL = 'alter table public.key_shapes add column if not exists image_url text;';

function isMissingColumnError(err) {
  const message = (err && (err.message || String(err))) || '';
  return /PGRST204|42703/i.test(message)
    || /column .*image_url.*does not exist/i.test(message)
    || /could not find the 'image_url' column/i.test(message);
}

// True when key_shapes.image_url exists. `mode` says how it was checked.
async function shapeImageColumnPresent() {
  if (driver() === 'sql') {
    const res = await pgPool().query(
      `select 1 from information_schema.columns
        where table_schema = 'public' and table_name = 'key_shapes' and column_name = 'image_url'`,
    );
    return { present: res.rowCount > 0, mode: 'sql' };
  }
  if (driver() === 'postgrest') {
    // Selecting the column itself is the probe: PostgREST answers PGRST204 /
    // 42703 when the migration has not been applied.
    const { error } = await sbClient().from('key_shapes').select('image_url').limit(1);
    if (!error) return { present: true, mode: 'postgrest' };
    if (isMissingColumnError(error)) {
      return { present: false, mode: 'postgrest', error: error.message };
    }
    throw new Error(pgRestError('select key_shapes.image_url', error));
  }
  // The JSON store has no columns — nothing to apply there.
  return { present: true, mode: 'json' };
}

// Which credential could run the additive DDL on this deployment, if any.
async function shapeImageDdlCredential() {
  if (DRIVER === 'json') return 'json';
  if (DRIVER === 'sql' && DB_URL) return 'postgres';
  if (DRIVER === 'postgrest') {
    if (process.env.SUPABASE_ACCESS_TOKEN && (process.env.SUPABASE_PROJECT_REF || projectRefFromUrl(SUPABASE_URL))) {
      return 'management-api';
    }
    // Some projects install a generic `exec_sql(query)` RPC (the pattern the
    // Supabase docs recommend for one-off DDL). Use it when it is there.
    try {
      const { error } = await sbClient().rpc('exec_sql', { query: 'select 1' });
      if (!error) return 'rpc:exec_sql';
    } catch (e) { /* not installed — that is the normal case */ }
  }
  return null;
}

let _shapeSchema = null;   // cached per process: { ok, mode, message }
async function ensureShapeImageColumn() {
  if (_shapeSchema && _shapeSchema.ok) return _shapeSchema;
  const probe = await shapeImageColumnPresent().catch((e) => ({ present: null, error: e.message }));
  if (probe.present === true) {
    _shapeSchema = { ok: true, mode: probe.mode, alreadyApplied: true };
    return _shapeSchema;
  }
  const credential = await shapeImageDdlCredential();
  if (!credential || credential === 'json') {
    _shapeSchema = {
      ok: false,
      mode: 'none',
      message: 'key_shapes.image_url is missing and this server holds no credential that can run DDL. '
        + 'Apply supabase/migrations/009_shape_images.sql in Supabase '
        + '(SQL Editor, or `npm run migrate:apply` with SUPABASE_DB_URL set).',
    };
    return _shapeSchema;
  }
  try {
    await runSql(SHAPE_IMAGE_DDL);
    _shapeSchema = { ok: true, mode: credential, applied: true };
    invalidate();
    console.log(`[shapes] key_shapes.image_url migration applied automatically via ${credential}.`);
  } catch (e) {
    _shapeSchema = { ok: false, mode: credential, message: e.message };
    console.warn('[shapes] could not apply the image_url column:', e.message);
  }
  return _shapeSchema;
}

// Everything Admin → Shapes needs to explain the state of the feature:
// the key_shapes rows (the exact columns the operator checks by hand), whether
// migration 009 is applied, and how many shapes carry an image.
async function shapeImagesStatus() {
  const out = {
    column: { present: null, mode: null, error: null },
    rows: [],
    rowCount: null,
    withImage: 0,
    error: null,
  ...
  try {
    out.column = await shapeImageColumnPresent();
  } catch (e) {
    out.column = { present: null, mode: null, error: e.message };
  }
  try {
    const rows = await selectRows('key_shapes', { order: { col: 'order', asc: true } });
    out.rows = rows.map((r) => {
      const app = mapping.rowToShape(r) || {};
      return {
        id: app.id || r.id,
        code: app.code || r.code,
        name_en: app.name_en || r.name_en || '',
        active: app.active !== false,
        image_url: (app.image_url || '') + '',
      };
    });
    out.rowCount = out.rows.length;
    out.withImage = out.rows.filter((r) => r.image_url).length;
  } catch (e) {
    out.error = e.message;
  }
  return out;
...
const JSON_COLLECTION = {
  products: 'products', categories: 'categories', key_shapes: 'shapes',
  brands: 'brands', bundles: 'bundles',
...
async function getCatalog() {
  ...
  // Shape management: the key-shape catalogue read is fault-tolerant — the
  // key_shapes table may not exist yet (migration 007 pending), and a
  // pending migration must never take the storefront down. In that case the
  // catalogue is DERIVED from the products' own key-shape flags, exactly the
  // pattern categories use, so every selector keeps the same A–D choices.
  let shapeRows = null;
  try {
    shapeRows = await selectRows('key_shapes', { order: { col: 'order', asc: true } });
  } catch (e) { /* not migrated yet — derive below */ }
  const shapes = shapeRows
    ? shapeRows.map(mapping.rowToShape)
    : mapping.deriveShapes({ products, brands: brandRows.map(mapping.rowToBrand) }).map(mapping.rowToShape);
...
function jsonCatalog() {
  ...
  return {
    ...
    shapes: jsonShapes(db),
    categories: jsonCategories(db),
  };
}

// The JSON dataset's key-shape catalogue (Admin → Shapes). Older datasets
// without a `shapes` collection get the SAME derivation the SQL driver and
// the content migration use, so every driver serves the same catalogue even
// before the Admin has ever opened the Shapes screen.
function jsonShapes(db) {
  const stored = mapping.normalizeShapes(Array.isArray(db.shapes) ? db.shapes : []);
  if (stored.length) return stored;
  return mapping.deriveShapes(db).map(mapping.rowToShape);
}
...
function normalizeStorefrontSettings(settings) {
  const s = Object.assign({}, settings || {}, {
    freeShippingThreshold: require('./shipping').FREE_SHIPPING_THRESHOLD,
  });
  let cfg = s.instapay;
...
function publicPayload(catalog) {
  return {
    ...
    // The key-shape catalogue the storefront renders its selectors from.
    // Hidden shapes are INCLUDED but flagged (active:false): the storefront
    // must be able to tell a catalogue shape an admin HID (blocked on the
    // PDP / Quick Add / Fitment / orders, like the server-side check) apart
    // from a product-local CUSTOM shape the catalogue doesn't manage at all
    // (still sellable). Every selector enumerating the catalogue filters
    // active !== false, so hidden shapes never appear anywhere.
    shapes: (catalog.shapes || []).slice().sort((a, b) => (a.order || 0) - (b.order || 0)),
...
async function getOrders(opts = {}) {
  ...
  if (driver() === 'json') {
    orderRows = jsonRows('orders', {}).map((o) => ({
      id: o.id, created_at: o.createdAt || o.created_at, customer: o.customer, items: o.items,
      subtotal: o.subtotal, bundle_discount: o.bundleDiscount !== undefined ? o.bundleDiscount : o.bundle_discount, delivery_fee: o.deliveryFee !== undefined ? o.deliveryFee : o.delivery_fee,
      ...
      total: o.total, status: o.status, status_history: o.statusHistory || o.status_history, payment: o.payment,
      payment_status: o.paymentStatus !== undefined ? o.paymentStatus : o.payment_status,
...
async function setOrderPaymentStatus(id, paymentStatus) {
  const n = await updateRow('orders', id, { payment_status: paymentStatus });
  invalidate();
  return n;
}
...
const CONFLICT_TARGET = {
  products: 'id', categories: 'id', key_shapes: 'id', brands: 'id', bundles: 'id',
...
async function saveRecord(collection, record) {
  ...
  // A shape is meaningless without its code — reject it on BOTH drivers with
  // a clear message before anything is stored (a codeless row also has no id).
  if (collection === 'shapes' && !row.code) throw new Error('Shape is missing a code');
  ...
  // Shapes (the Admin → Shapes catalogue) follow the same rule: the json
  // record is routed through the row mapping and back so the code is
  // upper-cased/trimmed and the id is the deterministic shape_<code> —
  // exactly what the SQL driver stores.
  let stored = driver() === 'json' ? Object.assign({}, record, { id: row.id }, jsonExtra) : row;
  if (collection === 'shapes' && driver() === 'json') stored = mapping.rowToShape(row);
...
async function saveSettings(patch) {
  patch = Object.assign({}, patch, { freeShippingThreshold: require('./shipping').FREE_SHIPPING_THRESHOLD });
  if (driver() === 'json') {
...
async function getAdminData() {
  ...
  const out = Object.assign({}, catalog, { orders, settings: normalizeStorefrontSettings(catalog.settings) });
...
const DATA_API = {
  ...
  getOrders, getOrder, createOrder, setOrderStatus, setOrderPaymentStatus,
...
module.exports = {
  ...
  shapeImageColumnPresent, ensureShapeImageColumn, shapeImagesStatus, isMissingColumnError,
  publicPayload,
  ...fallbackApi,
  COLLECTION_TABLE,

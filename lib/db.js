...
// The `categories` table is optional: a database created from the schema (or a
// JSON store without a `categories` collection) simply has no rows, while the
// products still carry their `category`. When that happens the categories are
// DERIVED from those products — the very same rule the JSON driver has always
// used — instead of reporting an empty catalogue. An empty list silently broke
// everything downstream that is driven by it, most visibly the Customize flow:
// no categories meant no Customize entry in the storefront navigation at all.
function catalogCategories(rows, source) {
  const stored = (rows || []).filter(Boolean);
  if (stored.length) return stored.map(mapping.rowToCategory);
...
async function saveRecord(collection, record) {
  ...
  const stored = driver() === 'json' ? Object.assign({}, record, { id: row.id }) : row;

...
async function saveRecord(collection, record) {
  ...
  // passes through productToRow's color normalization — run it here so HEX
  // values, names and enabled-flags are cleaned identically on BOTH drivers.
  const jsonExtra = (driver() === 'json' && collection === 'products' && record.colors !== undefined)
    ? { colors: mapping.normalizeColors(record.colors) } : {};

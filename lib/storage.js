...
// PUBLIC. A `shape-images` bucket somebody created by hand as private would
// silently break every stored image (the public URL the row holds answers 400),
// so the visibility is verified — and repaired — before every upload instead
// of assumed.
...
async function uploadShapeImage(dataUrl, opts = {}) {
  ...
  try {
    // The bucket must exist and be public — verified before every upload so a
    // bucket that was deleted (or flipped private) in the dashboard is repaired
    // instead of silently storing a URL that cannot render.
    await ensureShapeBucket();
    let { error } = await put();
    // The bucket can also disappear between the verification above and the
    // upload itself: recreate it and retry once.
    if (error && /bucket not found|does not exist/i.test(error.message || '')) {
      await ensureShapeBucket();
      ({ error } = await put());
    }
